...
for a
{
	if [[ $a =~ ^[1-9][0-9]*$ ]] ;then
		...
		for u in `history|sed -nE "s&^\s*([0-9]+)\s+$a\\$&\1&p"`
		{
			((D))||{
				let m=HISTCMD-u
				echo -ne Line '\033[42;1;37m'; echo -n `history $m|head -n1`; echo -e '\033[0m'
				b=$u
			}
			history -d $((u-D++))
		}
		if ! ((D)) ;then echo No history line matched. Did nothing;set --;continue 2
		elif ((D>1)) ;then echo -e and else after this, up to '\033[41;1;37m'$D lines have been erased'\033[0m'
...
if((u-D-b+3<17)) ;then
	if((HISTCMD-b+3<17)) ;then
		...
		history $((l=HISTCMD-b+D+3)) |head -n17
	fi
	let h=l-17
else
...
IFS=$'\n';i=;for l in `history`;{ [[ $l =~ ^\ *([0-9]+)\*?[^[:graph:]]*$ ]] &&history -d $((BASH_REMATCH[1]-i++));}
unset IFS
}
