...
for n ;{ unset ln e
 ...
 if [[ $n =~ ^([0-9]+)((=-?|-)([0-9][0-9]*)?)?$ ]];then
  ...
  ((u<l)) &&{ T=$u;u=$l;l=$T; }
  ((l<OF)) || ((u>HISTCMD)) &&{
   echo $n is beyond range, the history starting-end line number is $OF-$HISTCMD;continue;}
  n=$l-$u
  b="$((u-l+1)) lines were"
  ((l==u)) &&{
   n=$l ; b='line was';}
...
[[ $Z ]] &&{
 ...
 ((l<12)) && history $((11-F))
 history $L | head -$F
 echo -e "\e[40;1;32m  ...Here's where the found, deleted $b$M, as of searched string \e[41;1;37m$Z\e[m"
 let H=HISTCMD-u
 ((U=H>11? 11: H))
 ((H)) && history $H |head -$U
 let U=H-U
